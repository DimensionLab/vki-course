...
def Simulated_A(func,X0,X_Bounds,n_iter,t_I,a=1):
...
def Anim(X_k,Cost_k,func,x_1m,x_1M,x_2m,x_2M,n_p,Name_Video):
   ...
   for k in range(n_iter):
    ...
    ax2.set_ylim([-0.1*np.max(Cost_k),1.2*np.max(Cost_k)])
    ax2.set_yticks([])
...
X_k,Cost_k,X_best =Simulated_A(rosenbrock,X0,X_Bounds,n_iter,t_I,2)
