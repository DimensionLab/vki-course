...
Npopulation = 20 #size of the population
...
name_plot = '2020_hao_case_CD_xc_0.02'
#get name 
data = df.loc[df['name'] == name_plot]

plt.figure()
plt.semilogx(data['Pi1'],data['PiF_log'])
plt.grid()  
plt.show()          
...
pset.renameArguments(ARG0='Pi1', ARG1='Pi4')
...
def MSE(individual, points):
    ...
    for ii in range(len(points)):
        sqerr[ii] = (func(points.iloc[ii,0],points.iloc[ii,1]) - data['PiF_log'].iloc[ii])**2
        #print(sqerr[ii])
...
toolbox.register("evaluate", MSE, points=data[['Pi1','Pi4']])
...
plotpoints = data[['Pi1','Pi4']]

plt.figure()
plt.semilogx(data['Pi1'],data['PiF_log'],marker='x',linestyle='-')
for ii in range(len(plotpoints)):  
    plt.scatter(plotpoints.iloc[ii,0],bestfunct(plotpoints.iloc[ii,0],plotpoints.iloc[ii,1]),marker='.',color='k')
plt.grid()
plt.xlabel('Pi1')
plt.ylabel('PiF_log')
plt.show()
