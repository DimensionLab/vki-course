...
def MSE(individual, points):
    ...
    for ii in range(len(points)):
        sqerr[ii] = (func(points.iloc[ii]) - data['PiF_log'].iloc[ii])**2
        #print(sqerr[ii])
...
toolbox.register("evaluate", MSE, points=data['Pi1'])
...
for ii in range(len(plotpoints)):  
    plt.scatter(plotpoints.iloc[ii],bestfunct(plotpoints.iloc[ii]),marker='.',color='k')
plt.grid()
plt.xlabel('Pi1')
plt.ylabel('PiF_log')
plt.show()
